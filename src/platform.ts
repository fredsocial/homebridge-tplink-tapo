...
import type { TapoDeviceConfigInput, TplinkSmarthomeConfig } from './config';
...
import TapoCharacteristics from './tapo/custom-characteristics';
import { L510EAccessory } from './tapo/platformL510EAccessory';
import { L520EAccessory } from './tapo/platformL520EAccessory';
import { L530Accessory } from './tapo/platformL530Accessory';
import { P100Accessory } from './tapo/platformP100Accessory';
import { P110Accessory } from './tapo/platformP110Accessory';
...
export default class TplinkSmarthomePlatform implements DynamicPlatformPlugin {
  ...
  public customCharacteristics: Record<
    string,
  ...
  constructor(
    ...
    if (
      ...
      this.log.error(
        `homebridge-tplink-tapo requires homebridge >= 1.3.0. Currently running: ${api.serverVersion}`
      );
      throw new Error(
        `homebridge-tplink-tapo requires homebridge >= 1.3.0. Currently running: ${api.serverVersion}`
      );
    ...
    this.customCharacteristics = {
      ...Characteristics(api.hap.Characteristic),
      ...TapoCharacteristics(api.hap.Characteristic),
    };
  ...
  private discoverTapoDevices(): void {
    if (this.config.tapo.devices.length === 0) return;

    if (this.config.username === '' || this.config.password === '') {
      this.log.warn(
        'Skipping configured Tapo devices because username/password are not set'
      );
      return;
    }

    for (const device of this.config.tapo.devices) {
      this.registerTapoDevice(device);
    }
  }

  private registerTapoDevice(device: TapoDeviceConfigInput): void {
    const uuid = this.api.hap.uuid.generate(
      device.host + (device.name != null ? device.name : '')
    );
    const displayName = device.name ?? device.host;
    const existingAccessory = this.configuredAccessories.get(uuid);
    const category = TplinkSmarthomePlatform.isTapoLight(device)
      ? Categories.LIGHTBULB
      : Categories.OUTLET;
    const PlatformAccessoryCtor = this.api.platformAccessory;
    const accessory =
      existingAccessory ??
      new PlatformAccessoryCtor(displayName, uuid, category);
    ...
    this.createTapoAccessory(accessory, device);

    if (existingAccessory != null) {
      this.log.info(
        `Restoring configured Tapo accessory: ${chalk.blue(`[${displayName}]`)}`
      );
    ...
    this.log.info(
      `Adding configured Tapo accessory: ${chalk.blue(`[${displayName}]`)}`
    );
    this.api.registerPlatformAccessories(PLUGIN_NAME, PLATFORM_NAME, [
      accessory,
    ]);
  ...
  private createTapoAccessory(
    accessory: PlatformAccessory,
    device: TapoDeviceConfigInput
  ): unknown {
    const timeout = device.timeout ?? 2;
    const type = device.type?.toLowerCase() ?? 'plug';

    if (type === 'colorlight' || type === 'l530' || type === 'l530e') {
      return new L530Accessory(
        this.log,
        this,
        accessory,
        timeout,
    ...
    if (type === 'whitelight' || type === 'l520' || type === 'l520e') {
      return new L520EAccessory(
        this.log,
        this,
        accessory,
        timeout,
    ...
    if (type === 'light' || type === 'l510' || type === 'l510e') {
      return new L510EAccessory(
        this.log,
        this,
        accessory,
        timeout,
    ...
    if (type === 'powerplug' || type === 'p110') {
      return new P110Accessory(
        this.log,
        this,
        accessory,
        timeout,
  ...
  private static isTapoLight(device: TapoDeviceConfigInput): boolean {
