...
export interface TplinkSmarthomeConfigInput {
  ...
  /**
   * Manual list of Tapo devices.
   */
  tapoDevices?: Array<TapoDeviceConfigInput>;
  /**
   * Username used in the Tapo app.
   */
  username?: string;
  /**
   * Password used in the Tapo app.
   */
  password?: string;
...
export type TplinkSmarthomeConfig = {
  ...
  tapo: {
    username: string;
    password: string;
    devices: Array<TapoDeviceConfigInput>;
  };
};
...
function isDeviceConfigInput(value: unknown): value is DeviceConfigInput {
  return (
    ...
    (!('updateInterval' in value) ||
      typeof value.updateInterval === 'number') &&
...
function isTplinkSmarthomeConfigInput(
  ...
  return (
    ...
    (!('devices' in c) ||
      ...
      c.devices === undefined) &&
    (!('tapoDevices' in c) ||
      isArrayOfDeviceConfigInput(c.tapoDevices) ||
      c.tapoDevices === undefined) &&
...
export function parseConfig(
  ...
  const allManualDevices = c.devices ?? [];
  const isTapoDevice = (device: DeviceConfigInput): boolean =>
    device.type !== undefined || device.name !== undefined;
  const legacyDevices = allManualDevices
    .filter((device) => !isTapoDevice(device))
    .map(({ host, port }) => ({ host, port }));
  const tapoDevices = [
    ...allManualDevices.filter(isTapoDevice),
    ...(c.tapoDevices ?? []),
  ];
  ...
  return {
    ...
    discoveryOptions: {
      ...
      devices: legacyDevices.length > 0 ? legacyDevices : undefined,
    },
    tapo: {
      username: c.username,
      password: c.password,
